// POST /api/admin/requests/:id/needs-resubmit
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  await env.DB.prepare(
    `UPDATE exclusion_requests
     SET status='needs_resubmit', admin_comment=?, updated_at=datetime('now')
     WHERE id=?`
  ).bind(comment, id).run()
  return Response.json({ ok: true, id, status: 'needs_resubmit' })
}
